...
Ledger.Sheet = (function () {
    ...
    state.rows.forEach((row, i) => {
      ...
      state.columns.forEach(col => {
        ...
        if (col.type === 'image') {
          ...
          td.innerHTML = val
            ? `<img src="${val}" class="thumb" data-row="${row.id}" data-col="${col.id}" data-role="photo">`
            : `<button class="photo-add" data-row="${row.id}" data-col="${col.id}" data-role="photo">📷</button>`;
        } else {
  ...
  function wireRowEvents() {
    ...
    document.querySelectorAll('[data-role="photo"]').forEach(el => {
